...
public class Main {
	...
	private static void testNamedParameterTemplate() {
		try (ClassPathXmlApplicationContext context 
			  ...
			  paramsMap.put("dataAssunzione", new BigDecimal(20181127));
			  
			  List<Dipendente> dipendenti = 			  
			  jdbc.query("SELECT ID,COGNOME,NOME,DATA_ASSUNZIONE FROM DIPENDENTI WHERE DATA_ASSUNZIONE=:dataAssunzione",paramsMap, 
